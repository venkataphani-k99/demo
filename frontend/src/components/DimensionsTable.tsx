...
export const DimensionsTable: React.FC<DimensionsTableProps> = ({
  ...
  if (dimensions.length === 0) {
    return (
      <div className={`rounded-xl border overflow-hidden backdrop-blur-md shadow-xl ${bgCard}`}>
        <div className={`flex items-center justify-between border-b px-4 py-3 ${bgHeader}`}>
          <div className="flex items-center space-x-2">
            <Ruler className="h-4 w-4 text-cyan-500" />
            <h3 className={`text-sm font-bold ${textHeading}`}>Dimension Candidates &amp; TechDraw Placements</h3>
          </div>
        </div>
      ...
      <div className={`border-b px-4 py-3 space-y-3 ${bgHeader}`}>
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="flex items-center flex-wrap gap-2 text-[11px] font-mono">
            <span className={`flex items-center space-x-1 rounded px-2 py-0.5 border ${
              isLight ? 'bg-slate-100 text-slate-700 border-slate-200' : 'bg-slate-800/80 text-slate-300 border-slate-700'
            }`}>
              <Database className={`h-3 w-3 ${textMuted}`} />
              <span>Raw: <strong className={textHeading}>{rawCount}</strong></span>
            </span>
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 pt-1">
          ...
          <div className="flex flex-wrap items-center gap-2">
            {/* Status Filter */}
            <div className={`flex items-center space-x-1 p-1 rounded-lg border text-xs font-mono ${
            ...
            {uniqueViews.length > 0 && (
              <select
                ...
                className={`rounded-lg border px-2.5 py-1 text-xs font-mono focus:border-cyan-500 focus:outline-none ${
                  isLight ? 'bg-white border-slate-300 text-slate-800' : 'border-slate-700 bg-slate-900 text-slate-300'
                }`}
              >
        ...
        <table className="w-full text-left text-xs">
          <thead className={`sticky top-0 border-b z-10 ${bgThead}`}>
            <tr>
              ...
              <th className="py-2.5 px-3 font-bold">Value / Nominal</th>
              <th className="py-2.5 px-3 font-bold">Semantic Role</th>
              <th className="py-2.5 px-3 font-bold">Source Feature</th>
              <th className="py-2.5 px-3 font-bold">Entities</th>
              <th className="py-2.5 px-3 font-bold">View Placement</th>
