...
def load_step(file_path: str | Path) -> StepLoadResult:
    ...
    if path.suffix.lower() not in valid_extensions:
        raise InvalidExtensionError(
            f"Invalid file extension '{path.suffix}'. Supported extensions are: {', '.join(valid_extensions)}"
        )

    # 3. Check file size
    file_size = path.stat().st_size
    if file_size == 0:
        raise CadImportError(f"STEP file is empty (0 bytes): {path}")

    # 4. Extract header metadata
    metadata = parse_step_header(path)
    ...
    try:
        # 6. Import STEP geometry via FreeCAD Import module
        ...
        if not objects:
            raise CadImportError(f"No CAD objects were generated from STEP file: {path.name}")

        # Collect shapes from objects
        shapes = [obj.Shape for obj in objects if hasattr(obj, "Shape") and not obj.Shape.isNull()]
        if not shapes:
            raise CadImportError(f"No valid B-Rep shapes found in STEP model: {path.name}")

        # If multiple shapes exist, create compound if needed; otherwise take primary
        if len(shapes) == 1:
            primary_shape = shapes[0]
        else:
            primary_shape = Part.makeCompound(shapes)
        ...
        if isinstance(e, CadImportError):
            raise
        raise CadImportError(f"Failed to load STEP file '{path.name}': {str(e)}") from e
